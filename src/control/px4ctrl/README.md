...
## 前置知识
### 简要阅读PX4官方文档
PX4飞控**不是**开箱即用的，需要比较复杂的前期调试，px4官方提供了非常详尽的用户文档 docs.px4.io ，强烈推荐花半天浏览和阅读一下文档的大致内容和框架。在后续遇到飞控问题的时候，可以对应到文档的相关内容进行调试，或者善用官方文档的搜索栏。

常用内容：
* 基础PID调试：https://docs.px4.io/main/en/config_mc/pid_tuning_guide_multicopter_basic.html
* 进阶PID调试：https://docs.px4.io/main/en/config_mc/pid_tuning_guide_multicopter.html
* 飞机滤波设置：https://docs.px4.io/main/en/config_mc/filter_tuning.html
* 飞行日志分析：https://docs.px4.io/main/en/dev_log/flight_log_analysis.html
* 用ROS控制PX4：https://docs.px4.io/main/en/robotics/index.html
* 全部飞控参数：https://docs.px4.io/main/en/advanced_config/parameter_reference.html

### 了解硬件系统

如果对硬件相关完全不了解，一定要看完实验室的开源硬件课程，对硬件，对整个系统有一个完整的概念理解：
【【完结】从0制作自主空中机器人 | 开源 | 浙江大学Fast-Lab】 https://www.bilibili.com/video/BV1WZ4y167me

不同的硬件组件如何在飞控中进行配置？对应的官网文档：https://docs.px4.io/main/en/hardware/drone_parts.html

### 学飞飞机

**无人机很危险！！！手拿遥控器就要对无人机、对自己和周围的其他人负责！！！**
如果不会飞飞机，非常建议参与飞行驾照培训。
根据这个文档的介绍熟悉练手无人机的手控飞行：https://qhwukij0eg.feishu.cn/wiki/DB4mww7cCiLwULkbdtkckoCTnCu?from=from_copylink

## 飞控配置

### 固件烧写

用QGC烧写代码库中的 .px4 后缀的固件，文件名称对应的是不同的飞控版本。

kakute飞控默认不支持px4，需要先刷写BootLoader之后才能正常使用QGC刷固件，方法如下：
...
按住kakute飞控的按钮，连接电脑上电，依次运行：
```
dfu-util -a 0 --dfuse-address 0x08000000:force:mass-erase:leave -D holybro_kakuteh7mini_bootloader.bin
dfu-util -a 0 --dfuse-address 0x08000000 -D holybro_kakuteh7mini_bootloader.bin
```

### QGC基础设置

基本配置参考：https://docs.px4.io/main/en/config/#standard-configuration
...
#### 校准传感器、遥控器
   5通道设置为飞行模式切换,7通道设置为Emergency Stop(推荐);
   飞行模式的设置中，不需要额外单独设置offboard模式，因为offboard是上位机程序切换进入的，需要一定前置条件，
   最好不要在无线连接飞控的情况下进行传感器校准，校准可能出现极大误差

#### 校准电源和执行器
   如果存在电压反馈，建议设置满电4.2V，没电3.5V，如果不存在电池反馈，需要设置
   CBRK_SUPPLY_CHECK = 894281， 关闭非法电压的解锁检查（可能导致电池过放）。

   电调PWM校准项目只可能对PWM类型的电调生效，用于校准PWM电调的油门行程。

   执行器界面可以方便的设置电机在机体系下的位置，电机转向，电调协议等等。
   对于初次使用需要辨识电机编号和方向，右上角的`Identify & Assign Motors`非常有用。
   
   如果发现电机转向反了，请使用`Acturator Testing`下面的`Set Spin Direction`来设置电机反转，而不是通过点击图标中的电机转向来设置反转！如果是PWM电调，只能重焊电机线序才能调换转向。

#### 串口配置

   PX4 为许多飞行控制器端口定义了一份默认功能，这就是为什么可以将 GPS 模块插入标有 GPS1 的端口，将遥控器接收器插入RC IN，或将遥测模块插入TELEM 1，通常它们都可以工作。

   这里仅强调飞控使用串口连接机载电脑的配置方式，其余传感器或者其他设备，请查看：
   串行端口配置：https://docs.px4.io/main/en/peripherals/serial_configuration.html#serial-port-configuration
   MavLink外围设备：https://docs.px4.io/main/en/peripherals/mavlink_peripherals.html#mavlink-peripherals-gcs-osd-companion

   飞控连接上位机的通讯使用的是MAVLink，飞控内置了三个MavLink实例，取其中任意一个实例来配置机载电脑的串口参数都可以，以下用X代替MAVLink实例编号，并认为飞控使用telem2端口和机载电脑通讯：
   MAV_X_CONFIG = TELEM2
   MAV_X_MODE = Onboard
   MAV_X_RATE = 92160B/s
   SER_TEL2_BAUD = 921600 8N1
   以上设置中，串口波特率，与上位机MAVROS的launch文件中设定的波特率应当一致，RATE是最大的发送速率，理论上只需要填写为BAUD的1/10即可，不过我个人习惯填写的更大，比如填写921600B/s，确保通讯带宽确实足够。
   如果串口配置的速率错误，可能导致上位机指定某些MAVROS频率无法达到指定数值（比如IMU我们一般会在上位机提升到200Hz，串口配置错误可能会导致mavros运行没有数据，或者IMU数据到达不了200Hz）

   > 对于微空的飞控，为了确保IMU频率正常，还需要额外设置两个参数：
   > IMU_GYRO_RATEMAX = 2000Hz
   > IMU_INTEG_RATE = 400Hz

#### 状态估计设置
   
   PX4有两种状态估计器一种是EKF2（默认），一种是Q Estimator。简单来说，飞控不连接任何外部传感器，请选择Q Estimator，否则选择EKF2。以下是详细介绍和设置：

##### 使用EKF2

   使用PX4的导航滤波器（EKF2）：https://docs.px4.io/main/en/advanced_config/tuning_the_ecl_ekf.html#using-the-ecl-ekf

   EFK2支持进行多传感器融合，但是至少需要IMU数据、偏航数据源（磁力计或外部视觉）和高度数据源才能实现，然后可以使用其他传感器数据来估计其他状态。大部分飞控会标配磁力计和气压计作为运行EKF的必要传感设备，但是这些设备在低空小范围内飞行往往会有较大干扰，导致状态估计和控制不稳定，精度不高，且经常需要飞控重启来重置状态。好处就是可以使用定高（Attitude）和定点（Position）控制，因此更推荐在增加了额外传感器（比如光流、GPS）等场景下使用。
   SYS_MC_EST_GROUP = ekf2(recommended)

   EKF2可以在多传感器下同时运行多个实例，从而规避某个传感器错误的影响，但是多个EKF实例之间的切换可能导致无人机状态忽然跳变（比如偏航抽动）。因此目前还是推荐使用单个EKF实例：
   EKF2_MULTI_IMU = 0
   EKF2_MULTI_MAG = 0
   SENS_IMU_MODE = 1
   SENS_MAG_MODE = 1
   
   使用EKF做状态估计还需要根据不同的传感器设置不同的参数，在此仅列举光流参数设置，其余模块的设置请自行查看官网。

###### 举例：EKF2+光流

   光流使用向下的摄像头和向下的距离传感器来估算速度。它可用于在没有 GNSS 的情况下（在建筑物、地下或任何其他 GNSS 被拒绝的环境中）导航时确定速度。**PX4Ctrl中新增了依靠光流实现的安全保护功能，可以适当降低炸鸡概率，如有需要建议配置此部分内容。(目前代码还没移植过来)**

   光流需要物理插入飞控的某个指定串口，一般选择telem1或者telem2，具体的串口配置方法，需要根据不同的光流介绍，配置对应的串口参数，包括波特率等等，需要根据不同的光流手册进行配置，比如我使用的是优象科技的光流模块T2,其波特率为115200，假设我连接到了telem1端口，那么串口端的配置参数就是：
   MAV_X_CONFIG = TELEM1
   MAV_X_MODE = Normal
   
   之后在飞控端设置光流对应的波特率和传感器的方向：
   SER_TELx_BAUD = 115200 8N1 （x表示1或者2）
   SENS_FLOW_ROT = no rotation （前提是光流的默认方向和机头方向相同）

   在配置好光流串口之后，飞控的EKF2可以融合光流数据，需要至少配置以下参数：
   EKF2_OF_CTRL = 1
   EKF2_RNG_CTRL = 2 （在平坦地面飞行很有用）
   EKF2_HGT_REF = 2 

   配置成功后，飞机可以进行定高（Attitude）和定点（Position）飞行，但是飞行效果可能一般，这主要取决于环境地面的纹理，以及光流传感器本身的品质。请还是谨慎飞行。

   最后，因为使用了EKF2，多旋翼的降落检查器的降落判定时间会缩短，可能导致在有光流的情况下自动起飞不成功，（具体现象为自动起飞是解锁然后忽然自己锁定），修改降落检查器的触发时间大于3秒，这样无人机降落后自动锁定时间会更久：
   LNDMC_TRIG_TIME = 4

   > **EKF2+光流的精调参数** 
   > 尺度缩放：https://docs.px4.io/main/en/sensor/optical_flow.html#setup
   > SENS_FLOW_SCALE: integrated_x_gyro和integrated_x应该在纯旋转的时候完全一致，该参数负责调整光流的缩放。另外在极高空中悬停时缩小该参数也可以减缓震荡情况，因为光流分辨率较低
   > 位置偏移量：https://docs.px4.io/main/en/sensor/optical_flow.html#estimators
   > 设置相对偏移量：EKF2_OF_POS_X,Y,Z：在body系下光流角点的位置
   > EKF2_OF_DELAY：光流传感器相对于IMU的时延
   > EKF2_IMU_POS_X,Y,Z：IMU相对集体坐标系的位置
   > EKF2_OF_QMIN：光流数据的质量大于该数值才会被融入EKF中


##### 使用Q Estimator
   
   Q Estimator是一种非常简单的基于四元数的姿态互补滤波器。它不需要 GPS、磁力计或气压计等额外传感器，就能获得无人机比较准确的姿态信息，无需频繁重启飞控，但是也导致无法启动定高定点功能，只能使用姿态控制（Stabilized & Manual）或者角速度控制（Acro），这两种模式都是油门直通，即油门推杆直通飞控的油门输出，需要学会飞行驾照才能较好飞行。
   SYS_MC_EST_GROUP = Q attitude estimator(no position)  

   Q Estimator不需要其余传感器，因此可以直接关闭空速计，磁力计，这样对应传感器也无需校准。
   SYS_HAS_BARO = 0  
   SYS_HAS_MAG = 0 

#### 其他按需配置

以下配置不必须，且仅供参考，因为具体的配置参数很可能因为传感器的不同而不同。请自行搜索官网文档，了解参数对应功能的具体细节后自行判断。
...
##### 无遥控器飞行
   ...
   如果使用场景存在遥控器信号断连但仍然想要无人机自主控制，需要设置飞控在offboard模式下不会因遥控器信号丢失介入故障保护：COM_RCL_EXCEPT = 7

   不同遥控器和飞控对遥控器信号丢失的反应可能是不同的，这里**仅以乐迪遥控器作为例子**，说明失控保护的细节逻辑：
   * 遥控器和接收机：乐迪遥控器和接收机距离过远，接收机是知道信号断掉的，但是接收机作何反应取决于遥控器的失控保护如何设置。以乐迪AT9S遥控器为例，默认的失控保护功能设置为：除了油门通道油门值修改为15%以外，其他通道保持原有通道值不变。
   * 飞控：飞控和接收机相连，也会知道遥控器信号丢失，从而介入对应的保护模式中（进入什么模式取决于参数NAV_RCL_ACT），所以为了让飞机自主飞行（OFFBOARD模式下）不会因为遥控器信号丢失强制退出，需要设置参数COM_RCL_EXCEPT = 7
   * 上位机MAVROS：MAVROS中的rc/in消息会发送遥控器当前信号。根据测试，如果飞控上电后遥控器信号一直没有收到，则MAVROS的rc/in消息不会有任何输出，如果在上电后遥控器曾经连接过接收机，那么rc/in消息会根据遥控器的失控保护设置发送对应的信号。
   * PX4ctrl
     * no_rc=true：PX4Ctrl中支持无遥控器飞行，其原理为直接不接收rc/in的消息，意味着无遥控飞行的时候，哪怕有遥控器开着，PX4Ctrl的状态机都不会因为遥控器通道或者拨杆的位置进行改变，但是通过5通道切换飞控进入手控模式仍然可以，实现这种无遥控飞行只需要设置飞控的参数COM_RCL_EXCEPT即可，甚至可以完全不开遥控器。
     * no_rc=false：如果希望PX4Ctrl仍然开着遥控器飞行，但是会飞出遥控器的信号范围之外，除了设置飞控的参数COM_RCL_EXCEPT以外，还需要设置遥控器的失控保护维持所有通道数据不变（尤其是油门通道的15%需要改为NOR），这样rc/in会保持遥控器丢失时的1-4通道中位。否则飞机在飞出遥控器信号范围之外时，会因为遥控器的失控保护设置，让rc/in消息的油门通道值为15%，当PX4Ctrl执行完轨迹进入AUTO_HOVER模式时，就会导致飞机选停点Z轴高度变化，飞机就会缓慢向上或者向下飞行。

...
: runtime_(nvinfer1::createInferRuntime(gLogger_))
{
    ...
    if (isDimsSpecified(inDims) && isDimsSpecified(outDims))
    {
        // 有效维度 → 直接推导真实值
        inputCount_  = volume(inDims);
        outputCount_ = volume(outDims);
    ...
    {
        // 动态模型（含 -1），维度未指定，保持默认值
        currentBatch_         = 0;
        inputElemsPerSample_  = 0;
        outputElemsPerSample_ = 0;
    }
}
